...
/* ------------------ Star Tracker main functional program ----------------
    This is the main program, using the Adafruit OLED driver,
    MPU6050 with I2C dev, the built-in servo driver, and the
    SoftwareSerial library (any other serial device will work).

    The servo is not smooth, quite discrete, the oled sometimses went wrong,
    U8g2 is abandon because too long time consuming, servo went bad.

    The MPU6050 is a 6-axis accelerometer and gyroscope, it is used to
    measure the angular velocity and acceleration of the device.
    The MPU6050 is connected to the Arduino via I2C.
    (Not used in this version, but we put it as a remote motion controller)

    The 128x32 OLED display is connected to the Arduino via I2C, so we need to
    connect the SDA and SCL pins to the Arduino. The display is used to display
    the current position of the star tracker.
    ----------------------------------------------------------------------- */
